...
<div class="demo">
		...
		<p align="center">
		<?=ckeditor('download_add', '', $this->session->userdata('group'))?>&nbsp;<?=form_error('download_add')?>
		</p>
		<p>Author</p>
		<?$i = 0?>
		<?$iq = 0?>
		...
		<p align="center"><?=form_submit('add_download', 'Add Download')?></p>
		<?=form_close()?>
		...
		<td width="7%">Delete</td>
			...
			<?foreach($news->result() as $t):?>
				...
				<td><?=$t->Subject?></td>
				<td><?=$t->HTML?></td>
				<td><?=anchor('admin/cabal/download_edit/'.$t->Bil, 'EDIT', 'title = "EDIT DOWNLOAD '.$t->Bil.'"')?></td>
				<td><?=anchor('admin/cabal/download_del/'.$t->Bil, 'DELETE', 'title = "DELETE DOWNLOAD '.$t->Bil.'"')?></td>
				</tr>
			<?endforeach?>
		</table>
		</td>
